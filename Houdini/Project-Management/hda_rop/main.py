...
def cache_dir():
    ...
    if not root:
        root = hou.getenv('HIP')
        root = os.path.joint(root, 'Output')
...
def cache_path(mode='path'):
    ...
    if node.evalParm('trange') > 0:
        frame = '.{0:0>4d}'.format(int(hou.frame()))
    else:
        frame = ''
...
def watchlist_action(kwargs):
    ...
    for p in parent_node.parms():
        ...
        if parm_type == hou.parmTemplateType.Folder: #or hou.parmTemplateType.FolderSet:
            print('Foldeorset')
            continue
...
def watchlist_write():
    ...
    def set_string_attrib(attrib_name, attrib_val):
        if not geo.findGlobalAttrib(attrib_name):
            # print("No attrib with this name found")
    ...
    for n in range(num_watch_nodes):
        ...
        for p in range(num_watch_parms):
            ...
            if has_keyframe and not is_expr:
                val = ('<< Animated Param >>', '<< Animated Param >>')
            else:
                val = (watch_parm.eval(), watch_parm.evalAsString())

            watch_metadata[watch_node_path][key] = val

    # print("watch_metadata:", watch_metadata)
    _watch_metadata = json.dumps(watch_metadata, ensure_ascii=False)
...
def watchlist_read(kwargs):
    ...
    for i, (k, v) in enumerate(watchlist_dict.items()):
        ...
        for j, (k2, v2) in enumerate(v.items()):
            node.parm('read_watch_parm_{0}_{1}'.format(i+1, j+1)).set(k2)
            node.parm('read_watch_parm_{0}_{1}_val'.format(i+1, j+1)).set(v2[1])
...
def watchlist_restore_single(kwargs):
    ...
    if not parent_node:
        util.error('Node to restore to [{0}] does not exists'.format(parent_parm.evalAsString()))
        return
    ...
    if not parm_to_restore:
        util.error('Parm to restore [{0}/{1}] does not exists'.format(parent_node.path(), parm_name))
        return
...
def save_backup_hip(node):
    ...
    def generate_report(success, **kwargs):
        ...
        if success:
            util.print_report('Scene Backup Report',
                              'Status: Successfull',
    ...
    if len(delta) != 1:
        error_report('Unable to link the backup file')
        return
    ...
    cache_hip_dir = util.fix_path(os.path.join(cache_dir, 'cache_hips'))
    ...
    # cache_hip_path = cache_hip_path.replace('\\', '/')  To break and check for error log report

    try:
        shutil.copy(new_backup_path, cache_hip_path)
    except:
        error_report('Unable to copy the backup file to cache .hip file directory')
        return
    
    date = datetime.datetime.now()
    generate_report(True, scenefile=cache_hip_path, date=str(date))
    return
...
def hip_actions(action):
    ...
    if action == 'explore':
        # util.print_report('Opening Explorer', 'File: {0}'.format(hip_file))
...
def restore(kwargs):
    ...
    for n in hou.node('/obj').children():
        if n.type().nameComponents()[2] == 'scene_data':
            ...
            break
    if not scene_data:
        util.error('SCENE DATA node not found\n Scene cannot be correctly restored')
        return
        
    print(os.path.dirname(hou.hipFile.path()))
    if os.path.dirname(hou.hipFile.path()) == hou.getenv('SAVE_PATH'):
            
        message = 'Current .hip already seems to be saved to \"working files\" directory\n Are you sure you want to perform the operation? '
        choice = hou.ui.displayMessage(message, buttons=('OK', 'Cancel'), severity=hou.severityType.Warning)
        if choice == 1:
            return
    ...
    if not match:
        util.error('Source does not align with correct naming convention\nUnable to restore file correctly')
        return
        

    task = match.group('task')
    if not task or task == '':
        util.error('Unable to find task name from metadata\n Reverting to "restore" task name',
                   severity=hou.severityType.Warning)
        task = 'restore'
...
def onCreated(kwargs):
    ...
    node.parm('metadata_hipfile_path').lock(True)
